...
There are missing values. I would fix this by using replace_na.

  3. I would like to understand how much money I "saved" by gardening, for each vegetable type. Describe how I could use the `garden_harvest` and `garden_spending` datasets, along with data from somewhere like [this](https://products.wholefoodsmarket.com/search?sort=relevance&store=10542) to answer this question. You can answer this in words, referencing various join functions. You don't need R code but could provide some if it's helpful.

You could see how much each type of vegetable would cost to buy at the store and use left_join to join the garden_harvest and garden_spending datasets to see more clearly how much money you saved.
...
Trips %>%
  ...
  mutate(hour = hour(sdate),
        ...
        time = hour+min) %>%
  ggplot(aes(x = time)) +
  geom_density(fill = "purple", alpha = .5, color=NA) +
  theme(axis.text.y = element_blank()) +
  facet_wrap(vars(day))
```
...
Trips %>%
  ...
  mutate(hour = hour(sdate),
        ...
        time = hour+min) %>%
  ggplot(aes(x = time)) +
  geom_density(fill = "purple", alpha = .5, color=NA, position = position_stack()) +
  theme(axis.text.y = element_blank()) +
  facet_wrap(vars(day))
```
...
Trips %>%
  ...
  ggplot(aes(y = day)) +
  geom_density(fill = "darkblue", alpha = .5, color=NA) +
...
Trips %>%
  mutate(just_date = as_date(sdate)) %>%
  left_join(top_ten_station, by=c("sstation", "just_date"))
```
...
## GitHub link
  ...
  20. Below, provide a link to your GitHub page with this set of Weekly Exercises. Specifically, if the name of the file is 03_exercises.Rmd, provide a link to the 03_exercises.md file, which is the one that will be most readable on GitHub.
   ...
   https://github.com/kramakri622/03weeklyexercises
...
```{r}
kids %>%
  mutate(change_in_spending = cumsum(inf_adj)) %>%
